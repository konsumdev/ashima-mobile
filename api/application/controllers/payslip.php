...
class Payslip extends CI_Controller{
	...
	public function payslip_v2 () {
	    ...
	    $get_data_payslip = $this->filter_payslips($get_data_payslip);
	...
	public function filter_payslips($payslips)
	{
		if (!$payslips) {
			return false;
		}
		...
		foreach ($payslips as $row) {
			if ($row) {
				$check_approval_date = $this->mobile->reverse_check_approval_date($row->payroll_date,$row->period_from,$row->period_to,$this->company_id,$row->emp_id, $row->payroll_group_id);
	...
	public function payslip_detail_v2 () {
	    ...
	    if($flag_payslip == 1) {
	        ...
	        if($get_current_payslip) {
                ...
                $draft = check_employee_paydraft($this->emp_id,$get_current_payslip->payroll_group_id,$this->company_id);
                
                if ($draft) {
                    $is_draft =true;
                    foreach($draft as $drow) {
                        $payroll_payslip_id = $drow->payroll_payslip_id;
                    }
                } else {
	    ...
	    if($payroll_payslip_id) {	    
                ...
                $payslip_settings = $this->pp3->get_payslip_settings($this->company_id);
				...
				$gen_payslip = $this->pp3->payroll_run_details_where($payroll_run_details->payroll_payslip_id,$this->emp_id,$this->company_id);
				...
				$pp_sss = $gen_payslip ? $gen_payslip->pp_sss : 0;
				if (!$pp_sss) {
					$pp_sss = $payroll_run_details ? $payroll_run_details->pp_sss : 0;
				}
                ...
                if($this_other_deductions){
                    foreach ($this_other_deductions as $row_other_deductions) {
                        ...
                        $other_deduction_name = $row_deductions->deduction_name;
        	    ...
        	    $adjustments_array = array(
        	        "adjustment_overtime" => ($this_overtime_amount > 0) ? $this_overtime_amount : 0,
        	        "adjustment_holiday" => ($this_holiday_amount > 0) ? $this_holiday_amount : 0,
        	        "adjustment_night_diff" => ($this_night_diff_amount > 0) ? $this_night_diff_amount : 0,
        	        "adjustment_leave" => ($this_paid_leave_amount > 0) ? $this_paid_leave_amount : 0,
        	        "adjustment_allowance" => ($this_allowance_amount > 0) ? $this_allowance_amount : 0,
        	        "adjustment_commission" => ($this_commission_amount > 0) ? $this_commission_amount : 0,
        	        "adjustment_advance_pay" => ($this_advance_payment_amount > 0) ? $this_advance_payment_amount : 0,
        	        "adjustment_de_minimis" => ($this_deminimis_amount > 0) ? $this_deminimis_amount : 0,
        	        "adjustment_service_charge" => ($this_service_charge_amount > 0) ? $this_service_charge_amount : 0,
        	        "adjustment_hazard_pay" => ($this_hazard_pay_amount > 0) ? $this_hazard_pay_amount : 0,
        	        "adjustment_workday" => ($this_workday_amount > 0) ? $this_workday_amount : 0
        	    );
        	    ...
        	    $payslip_result = array(
                    ...
                    "ps_settings" => $payslip_settings
        	    );
        	    
        	    echo json_encode(array("result" => "1", "is_draft"=>$is_draft, "payslip" => $payslip_result));
        	    return false;
	    } else {
	        echo json_encode(array("result" => "0"));
	        return false;
