...
class Payroll_run_model extends CI_Model {
	...
	public function get_leave_application($company_id,$payroll_group_id,$date_start,$date_end,$offset=10,$limit=0,$sort_by,$q) {
		if(is_numeric($company_id)) { 
			...
			if($q !==""){
				$this->db->where("
					((CONCAT(AES_DECRYPT(employee.first_name,'{$konsum_key}'),' ',AES_DECRYPT(employee.last_name,'{$konsum_key}')) LIKE '%".$q."%')
	...
	*/
		...
		if($check_employee_period_type != FALSE){
			...
			if($check_minimum_wage_earner != FALSE){
				if ($check_minimum_wage_earner->minimum_wage_earner == 'yes') {
					...
					if($employee_period_type == "Semi Monthly" && $employee_pay_rate_type == "By Hour"){ // hourly
						return ($day_per_year > 0) ? $bp : 0 ;
					}else if($employee_period_type == "Semi Monthly" && $employee_pay_rate_type == "By Day"){ // daily
						// return ($day_per_year > 0) ? ($bp / 8) : 0 ;
						return ($day_per_year > 0) ? ($bp / $average_working_hours_per_day) : 0 ;
	...
	public function get_deduction_income_tax($type,$status,$salary_range)
	{
		...
		$valid_type = array('Daily','Semi Monthly','Monthly','Annual');
		$tax_status = array(
			-1 => 'Z',
			0  => 'S',
			1  => 'S-1',
	...
	public function payroll_run_employee($payroll_group_id,$company_id,$per_page, $page, $sort_by, $q, $sort_param=NULL){
		...
		if($q !==""){
			$this->db->where("
				epl.payroll_group_id = {$payroll_group_id} AND e.company_id = {$company_id} AND e.status = 'Active' AND 
				epl.employee_status = 'Active' AND epl.tax_status >= -1 AND
				(CONCAT(AES_DECRYPT(e.first_name,'{$konsum_key}'),' ',AES_DECRYPT(e.last_name,'{$konsum_key}')) LIKE '%".$q."%')
				OR AES_DECRYPT(a.payroll_cloud_id,'{$konsum_key}')  LIKE '%".$q."%'
				", NULL, FALSE); // encrypt
		}
	...
	public function custom_payroll_run_employee($draft_pay_run_id,$company_id,$per_page, $page, $sort_by, $q, $sort_param=NULL){
		...
		if($q !==""){
			$this->db->where("
					...
					AND prc.company_id = {$company_id} AND prc.status = 'Active' AND
					epl.employee_status = 'Active' AND epl.tax_status >= -1 AND
					(CONCAT(AES_DECRYPT(e.first_name,'{$konsum_key}'),' ',AES_DECRYPT(e.last_name,'{$konsum_key}')) LIKE '%".$q."%')
					OR AES_DECRYPT(a.payroll_cloud_id,'{$konsum_key}')  LIKE '%".$q."%'
				", NULL, FALSE); // encrypt
	 ...
	 * @param unknown_type $q
	 */
	public function payroll_run_employee_counter($payroll_group_id,$company_id,$q){
		$konsum_key = konsum_key();
		$s = array(
			"COUNT(e.emp_id) AS total"
		);
	 ...
	 * @param unknown_type $q
	 */
	public function custom_payroll_run_employee_counter($draft_pay_run_id,$company_id,$q){
		$konsum_key = konsum_key();
		$s = array(
				"COUNT(e.emp_id) AS total"
		);
		...
		if($q !==""){
			$this->db->where("
					prc.draft_pay_run_id = {$draft_pay_run_id} 
					AND e.company_id = {$company_id} AND e.status = 'Active' AND
					epl.employee_status = 'Active' AND epl.tax_status >= -1 AND 
					(CONCAT(AES_DECRYPT(e.first_name,'{$konsum_key}'),' ',AES_DECRYPT(e.last_name,'{$konsum_key}')) LIKE '%".$q."%')
					OR AES_DECRYPT(a.payroll_cloud_id,'{$konsum_key}')  LIKE '%".$q."%'
				", NULL, FALSE); // encrypt
	...
	public function get_employee_government_loans($emp_id,$id,$period_from,$period_to,$payroll_period){
		...
		$this->db->where($w);
		$this->db->join("gov_loans_deduction AS gld","gld.loan_deduction_id = prg.loan_deduction_id","INNER");
	 ...
	 * @param unknown_type $q
	 */
	public function employee_master_list_counter($payroll_period,$period_from,$period_to,$company_id,$q){
		$konsum_key = konsum_key();
		$s = array(
			"COUNT(pp.emp_id) AS total"
		);
	 ...
	 * @param unknown_type $company_id
	 */
	public function check_employee_payroll_commission($emp_id,$company_id){
		$month = date("m");
		$year = date("Y");
		$w = array(
			"YEAR(payroll_period)" => $year,
	...
	public function employee_calculate_income($earnings,$company_id,$payroll_group_id,$employee_id,$period,$work_schedule_id=NULL)
	{
		...
		$extra_pay_net =  $overtime_rate + $night_diff_rate + $holiday_rate - $absent_rate - $tardiness_rate - $undertime_rate + $total_carry_over + $remaining_taxable_allowances + $advance_amount 
		+ $minimum_wage_earner_cola + $de_minimis_excess + $service_charge_taxable + $hazard_pay_taxable + $employee_other_earnings_taxable;
		// $extra_pay_net = ($extra_pay_net < 0) ? 0 : $extra_pay_net ;
		
		$extra_pay_regular = $absent_rate - $tardiness_rate - $undertime_rate + $remaining_taxable_allowances + $advance_amount + $minimum_wage_earner_cola 
		+ $de_minimis_excess + $service_charge_taxable + $hazard_pay_taxable + $employee_other_earnings_taxable;
		// $extra_pay_regular = ($extra_pay_regular < 0) ? 0 : $extra_pay_regular ;
	 ...
	 * @param unknown $period_to
	 */
	public function check_flexible_overtime_date($emp_id,$company_id,$date,$pay_period,$period_from,$period_to){
		$w = array(
			"company_id"=>$company_id,
			"emp_id"=>$emp_id,
	...
	public function get_employees($employee_name,$company_id,$pay_schedule_dp,$pay_calculation_dp){
		...
		$this->db->where("
				...
				(CONCAT(AES_DECRYPT(e.first_name,'{$konsum_key}'),' ',AES_DECRYPT(e.last_name,'{$konsum_key}')) LIKE '%".$employee_name."%')
	...
	public function check_payroll_payslip($params){
		...
		if($r){
			...
			foreach($r as $row){
				...
				$check_payroll_period = $this->check_payroll_period_for_approval_for_email($check_payroll_period_params);
	 ...
	 * @param unknown $company_id
	 */
	public function check_employee_excluded($company_id){
		$w = array(
			"exclude"=>"1",
			"company_id"=>$company_id
		);
	...
	public function new_employee_leave_application($leave_date_start,$emp_id,$company_id){
		$w = array(
			...
			"company_id"=>$company_id,
			"status"=>"Active",
			"DATE(date_start) <= " => $leave_date_start,
			"DATE(date_end) >= " => $leave_date_start,
			"leave_application_status" => "approve"
		);
		$this->db->where($w);
		$this->db->where("(flag_parent != 'yes' OR flag_parent = 'no' OR flag_parent IS NULL)");		
		$q = $this->db->get("employee_leaves_application");
		$r = $q->row();
		return ($r) ? $r : FALSE ;
	}
